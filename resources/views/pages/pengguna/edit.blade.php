<x-app-layout>
    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        <form action="/user/{{$user->id_user}}" method="post" class="grid grid-cols-2 gap-6 mt-2">
            @method('PUT')
              @csrf
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                ...
                <select name="hak_akses" id="hak_akses" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-rose-500 focus:ring focus:ring-rose-500 focus:ring-opacity-50">
                    ...
                    <option value="admin keuangan" {{ $user->hak_akses === 'admin keuangan' ? 'selected' : '' }}>Admin Keuangan</option>
                    <option value="admin administratif" {{ $user->hak_akses === 'admin administratif' ? 'selected' : '' }}>Admin Administratif</option>
